/**
 * Intent Classifier — Detects patient intent from transcript text.
 * Used for dynamic mid-call routing (symptoms, appointment, callback, interrupt, off-topic).
 */
...
/**
 * Classify patient's spoken text into an intent.
 * Keywords are Arabic + English with legacy compatibility.
 */
export function classifyIntent(text: string): PatientIntent {
    ...
    const diseasePatterns = [
        'ألم', 'حمى', 'صداع', 'سعال', 'حرارة', 'دوخة', 'مفصل', 'ظهر', 'جلد', 'عين',
    ...
    // --- Book appointment ---
    const buyPatterns = [
        'موعد', 'حجز', 'احجز', 'أريد موعد', 'appointment', 'book', 'doctor', 'عيادة',
    ...
    // --- Decline booking (polite refusal) ---
    const declinePatterns = [
        'لا أريد', 'لا الآن', 'لاحقاً', 'غير مهتم',
...
// Backward-compat alias for any external imports during transition.
export type FarmerIntent = PatientIntent;
